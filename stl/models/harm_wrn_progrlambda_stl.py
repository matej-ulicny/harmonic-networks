"""
    Definition of full harmonic Wide Residual Network with progressive compression.
...
def resnet(depth, width, num_classes, dropout, level=None):
    ...
    def gen_block_params(ni, no, lvl=None):
        return {
            'harmonic0': gen_harmonic_params(ni, no, k=3, normalize=False, level=lvl, linear=True),
            'harmonic1': gen_harmonic_params(no, no, k=3, normalize=False, level=lvl, linear=True),
            'bn0': utils.bnparams(ni),
            'bn1': utils.bnparams(no),
            'convdim': utils.conv_params(ni, no, 1) if ni != no else None,
        }

    def gen_group_params(ni, no, count, lvl=None):
        return {'block%d' % i: gen_block_params(ni if i == 0 else no, no, lvl)
                for i in range(count)}

    flat_params = utils.cast(utils.flatten({
        'dct0': utils.dct_filters(n=3, groups=3),
        'dctfull': utils.dct_filters(n=3, groups=int(width)*16, expand_dim=0, level=None),
        'dctl3': utils.dct_filters(n=3, groups=int(width)*32, expand_dim=0, level=3),
        'dctl2': utils.dct_filters(n=3, groups=int(width)*64, expand_dim=0, level=2),
        'harmonic0': gen_harmonic_params(3, 16, k=3, normalize=True, level=None),
        'group0': gen_group_params(16, widths[0], n, lvl=None),
        'group1': gen_group_params(widths[0], widths[1], n, lvl=3),
        'group2': gen_group_params(widths[1], widths[2], n, lvl=2),
    ...
    def lin_harmonic_block(x, params, base, mode, stride=1, padding=1):
        ...
        if "group1" in base:
            ...
            filt_ind = "dctl2"
        filt = torch.sum(params[base + '.conv'] * params[filt_ind][:x.size(1), ...], dim=2)
