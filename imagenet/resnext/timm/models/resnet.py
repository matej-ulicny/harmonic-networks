...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
                 ...
                 norm_layer=nn.BatchNorm2d, drop_connect_rate=0, harmonic=False):
        ...
        self.conv1 = Harm2d(inplanes, first_planes, kernel_size=3, stride=stride, padding=dilation, bias=False) if harmonic else nn.Conv2d(inplanes, first_planes, kernel_size=3, stride=stride, padding=dilation, dilation=dilation, bias=False)
        self.bn1 = norm_layer(first_planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = Harm2d(first_planes, outplanes, kernel_size=3, padding=previous_dilation, bias=False) if harmonic else nn.Conv2d(first_planes, outplanes, kernel_size=3, padding=previous_dilation, dilation=previous_dilation, bias=False)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
        ...
        self.bn1 = norm_layer(first_planes)
        self.conv2 = Harm2d(first_planes, width, kernel_size=3, stride=stride, padding=dilation, groups=cardinality, bias=False) if harmonic else nn.Conv2d(first_planes, width, kernel_size=3, stride=stride, padding=dilation, dilation=dilation, groups=cardinality, bias=False)
...
class ResNet(nn.Module):
    ...
    root_pool : str, default None
        Pooling type after the first layer. One of 'avg', 'max', or None
    zero_init_residual : bool, default False
        Zero-initialize the last BN in each residual branch,
        so that the residual branch starts with zeros, and each residual block behaves like an identity.
        This improves the model by 0.2~0.3% according to https://arxiv.org/abs/1706.02677
    drop_connect_rate : float, default 0
        Base rate to drop out residual connections
    harmonic : bool, default False
        Replace convolutional layers with harmonic blocks
    """
    def __init__(self, block, layers, num_classes=1000, in_chans=3, use_se=False,
                 ...
                 norm_layer=nn.BatchNorm2d, drop_rate=0.0, global_pool='avg', root_pool=None, 
                 zero_init_residual=False, drop_connect_rate=0, harmonic=False):
        ...
        if deep_stem:
            if harmonic:
                self.conv1 = nn.Sequential(*[
                    Harm2d(in_chans, stem_width, 3, stride=2, padding=1, bias=False),
                    norm_layer(stem_width),
                    nn.ReLU(inplace=True),
                    Harm2d(stem_width, stem_width, 3, stride=1, padding=1, bias=False),
                    norm_layer(stem_width),
                    nn.ReLU(inplace=True),
                    Harm2d(stem_width, self.inplanes, 3, stride=1, padding=1, bias=False)])
            else:
                self.conv1 = nn.Sequential(*[
            ...
            self.conv1 = Harm2d(in_chans, stem_width, kernel_size=7, stride=root_stride, padding=3, bias=False, use_bn=True) if harmonic else nn.Conv2d(in_chans, stem_width, kernel_size=7, stride=root_stride, padding=3, bias=False)
        ...
        largs = dict(use_se=use_se, reduce_first=block_reduce_first, norm_layer=norm_layer,
                     avg_down=avg_down, down_kernel_size=down_kernel_size, num_blocks=sum(layers), 
                     max_drop_connect_rate=drop_connect_rate, harmonic=harmonic)
        self.layer1 = self._make_layer(block, 64, layers[0], stride=1, idx=0, **largs)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, idx=layers[0], **largs)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=stride_3_4, dilation=dilation_3, idx=sum(layers[:2]), **largs)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=stride_3_4, dilation=dilation_4, idx=sum(layers[:3]), **largs)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, reduce_first=1,
                    ...
                    idx=0, num_blocks=1, max_drop_connect_rate=0, harmonic=False):
        downsample = None
        ...
        idx += 1
        drop_connect_rate = max_drop_connect_rate
        drop_connect_rate *= float(idx) / num_blocks
        layers = [block(
            self.inplanes, planes, stride, downsample,
            cardinality=self.cardinality, base_width=self.base_width, reduce_first=reduce_first,
            use_se=use_se, dilation=first_dilation, previous_dilation=dilation, norm_layer=norm_layer, 
            drop_connect_rate=drop_connect_rate, harmonic=harmonic)]
