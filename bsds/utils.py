...
'''
def load_vgg16pretrain(model):
    vgg16 = models.vgg16(pretrained=True).state_dict()
    torch_params = model.state_dict()
    mapping = {"0": "conv1_1", "2": "conv1_2", "5": "conv2_1", 
               "7": "conv2_2", "10": "conv3_1", "12": "conv3_2", 
               "14": "conv3_3", "17": "conv4_1", "19": "conv4_2", 
               "21": "conv4_3", "24": "conv5_1", "26": "conv5_2", 
               "28": "conv5_3"}
    for k in vgg16.keys():
        name_par = k.split('.')
        if name_par[0] == 'features':
            name_space = mapping[name_par[1]] + '.' + name_par[2]
            torch_params[name_space] = vgg16[k]
    model.load_state_dict(torch_params)


def load_harm_vgg16pretrain(model):
    vgg16 = torch.load('harm_vgg16_conv.pth')
    torch_params = model.state_dict()
    mapping = {"0": "conv1_1", "2": "conv1_2", "5": "conv2_1", 
               "7": "conv2_2", "10": "conv3_1", "12": "conv3_2", 
               "14": "conv3_3", "17": "conv4_1", "19": "conv4_2", 
